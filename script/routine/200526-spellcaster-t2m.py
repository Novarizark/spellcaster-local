...
# ------------------------ Data Loading ----------------------
    ...
    for idx, row in sta_df.iterrows():
        ...
        if lasso_wgt > 0.0:
            ...
            # giss deg in anom, also combined dyn forecast, treat it as lag1 (fcst mon = curr mon) when conducting lag generation
            df_giss=pd.read_csv(giss_lib_dir+sta_num+'.t2m.csv', index_col=0, parse_dates=True)
            df_giss=df_giss[df_giss.index.year>=start_year]
            ...
            predict_Y=predict_Y*np.std(df_lbl.values)/(np.std(predict_Y)+1e-16)
        # end if 
