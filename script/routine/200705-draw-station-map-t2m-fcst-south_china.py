...
def main():
    ...
    #fcst_data='fcst.I200626.P2007.t2m.csv'
    ...
    ax.scatter( lon_dic['p2'], lat_dic['p2'],marker='.', color='darkred', 
            s=100,zorder=99, transform=ccrs.Geodetic(), label='>2.0℃ Sta_Num:'+str(len(lon_dic['p2'])))
    # +
    ax.scatter( lon_dic['p1'], lat_dic['p1'],marker='.', color='red', 
            s=50,zorder=10, transform=ccrs.Geodetic(), label='1.0~2.0℃ Sta_Num:'+str(len(lon_dic['p1'])))
    # +o
    ax.scatter( lon_dic['p0'], lat_dic['p0'],marker='.', color='gold', 
            s=30,zorder=2, transform=ccrs.Geodetic(), label='0.0~1.0℃ Sta_Num:'+str(len(lon_dic['p0'])))
    # -o
    ax.scatter( lon_dic['p-0'], lat_dic['p-0'],marker='.', color='skyblue', 
            s=40,zorder=2, transform=ccrs.Geodetic(), label='-1.0~0.0℃ Sta_Num:'+str(len(lon_dic['p-0'])))
    # -
    ax.scatter( lon_dic['p-1'], lat_dic['p-1'],marker='.', color='blue', 
            s=60,zorder=10, transform=ccrs.Geodetic(), label='-2.0~-1.0℃ Sta_Num:'+str(len(lon_dic['p-1'])))
    # --
    ax.scatter( lon_dic['p-2'], lat_dic['p-2'],marker='.', color='darkblue', 
            s=120,zorder=99, transform=ccrs.Geodetic(), label='<-2.0℃ Sta_Num:'+str(len(lon_dic['p-2'])))
    ...
    plt.savefig("../../fig/realtime.t2m.south_china.png", dpi=200, bbox_inches='tight')
#    plt.show()
